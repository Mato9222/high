...
public class SetTest {
	public static void main(String[] args) {
		...
		// Set에 중복되는 데이터를 추가하면 false 반환하고 데이터는 추가되지 않는다.
		boolean isAdd = hs1.add("FF");
		System.out.println("증북되지 않을 때 : " + isAdd);
		System.out.println("Set의 데이터 : " + hs1);
		System.out.println();

		isAdd = hs1.add("CC");
		System.out.println("증북될 때 : " + isAdd);
		System.out.println("Set의 데이터 : " + hs1);
		System.out.println();

		// Set의 데이터 수정하기
		// ==> Set에는 수정하는 명령이 따로 없기 떄문에 수정하려는 데이터를 삭제한 후
		// 새로 추가해 주는 방법으로 처리한다.
...
//		System.out.println(hs1);
		...
		/*
		 * Set의 데이터들은 순서(index)가 없기 떄문에 List처럼 index로 데이터를 하나씩 불러올 수 없다. 그래서 데이터를 하나씩 얻기
		 * 위해서는 iterator형의 객체로 반환해야 한다.
		 * 
		...
		// 데이터가 있으면 true, 데이터가 없으면 falise를 반환한다.

		// Iteratordml next() 메서드
		// ==> Iterator의 포인터를 다음번째 위치로 이동한 후 위치에 있는 데이터를
		// 읽어서 반환한다.
		while (it.hasNext()) {
