...
public class LprodMybatisTest {
	...
	//=========================================================================
		...
		// 원하는 SQK 문을 호출해서 실행하는 객체는 SqlSession객체이다.
		...
		try {
			...
			if(insertCnt > 0 ) {
				// SqlSession객체를 생성할 떄 AutoCommit이 비활성화 된 상태일 때는 
				// commit을 직접 실행해야 한다.
				session.commit();
...
/*
		...
		try {
			...
			if(updateCnt > 0 ) {
				...
				System.out.println("update 작성 실패..");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	/*
		...
		try {
			...
			// 형식) SqlSession객체.delete("namespce속성값.id속성값", 파라미터클래스)
			// 		반환값 : 작업에 성공한 레코드 수 
	...
	*/
		...
		System.out.print("검색 할 Lprod_GU 값 입력 >> ");
		...
		try {
			...
			// 원하는 select문을 호출하여 실행한다.
			// 	==> select문을 실행한 결과가 1개의 레코드인 경우에는 selectOne()
			//		메서드를 사용한다.
			//		selelctOne()메서드는 검색할 자료가 없으면 null을 반환한다.
